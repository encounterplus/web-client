...
/**
 * The drawn artwork of an asset: a still image, an animated sprite sheet or a video.
 *
 * Both kinds take `anchor`, `width`, `height`, `tint`, `alpha` and `filters`, so a view can place
 * one without knowing which it has. A video holds a shared decoder: `destroy()` it when done.
 */
export type AssetSprite = PIXI.AnimatedSprite | SplitAlphaVideo
